  ...
  return (
    <div className="gap-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mb-8">
      {cards.map((card) => {
        ...
        const allMonths = last12MonthsLeaveData.map((month, i) => ({
          month,
          days: usageByMonth[i] ?? 0,
          index: i,
        }));
        const maxDaysInMonth = Math.max(1, ...allMonths.map((m) => m.days));
        const maxScale = card.isFixedEvent
          ? Math.max(1, card.used ?? 0, maxDaysInMonth)
          : Math.max(1, card.totalAllocation ?? 1, maxDaysInMonth);
        ...
        return (
          ...
          >
            ...
            <div className="flex min-h-[52px] items-end justify-between gap-3">
              ...
              <div
                className="flex h-10 min-w-20 shrink-0 items-end justify-end gap-0.5"
                role="img"
                aria-label={`Leave usage last 1 year for ${card.title}`}
              >
                {allMonths.map(({ month, days, index }) => {
                  const pctHeight = maxScale > 0 ? (days / maxScale) * 100 : 0;
                  ...
                  return (
                    <Tooltip key={index}>
                      <TooltipTrigger asChild>
                        <div
                          className="w-1 rounded-sm cursor-default transition-opacity hover:opacity-100"
                          style={{
                            height: `${barHeightPct}%`,
